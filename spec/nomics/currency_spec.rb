...
RSpec.describe Nomics::Currency do
  ...
  describe ".where" do
    ...
    it "handles 401 unauthorized response code" do
      ...
      expect { described_class.fetch(ids: ["BTC"]) }.to raise_error(Nomics::BadApiKey)
    end
    ...
    it "handles non-200 response codes as errors" do
      ...
      expect { described_class.fetch(ids: ["BTC"]) }.to raise_error(Nomics::UnknownError)
    end
    ...
    it "requests currency tickers specified in ids argument" do
      ...
      described_class.fetch(ids: %w[ACOIN BCOIN])
    end
    ...
    it "returns empty list if given currency ticker does not exist" do
      ...
      expect(described_class.fetch(ids: ["YOY"])).to eq []
    end
    ...
    it "parses json and returns currency object" do
      ...
      coins = described_class.fetch(ids: ["BTC"])
    ...
    it "allows setting reference/fiat currency for price details" do
      ...
      described_class.fetch(ids: ["BTC"], convert: "GBP")
    end
  end
end
